...
class Filesystem {
	...
	public static function start() {
		...
		add_filter(
			'filesystem_method',
			[ '\Inc2734\WP_Google_Fonts\Filesystem', '_filesystem_method' ]
		);
	...
	public static function put_contents( $filepath, $contents ) {
		...
		if ( ! $result ) {
			throw new \RuntimeException( sprintf( '[inc2734/wp-google-fonts] Failed to write to %1$s.', $filepath ) );
		}
	...
	public static function rmdir( $filepath ) {
		...
		if ( ! $result ) {
			throw new \RuntimeException( sprintf( '[inc2734/wp-google-fonts] Failed to remove to %1$s.', $filepath ) );
		}
	...
	public static function mkdir( $target ) {
		...
		if ( ! $result ) {
			throw new \RuntimeException( sprintf( '[inc2734/wp-google-fonts] Failed to mkdir to %1$s.', $target ) );
		}
